...
void obhod_v_shirinu(node* n,int razm)
{
	node* tmp=n;
	node* tmp3=n;
	node* tmp2=NULL;// glubina количество уровней дерева
...
*/
void obhod_obratniy(node* root)
{
	if (root)
	{
		obhod_obratniy(root->left);
		obhod_obratniy(root->right);
		printf("%d ",root->value);
	}
}
...
int main(void)
{
	int i,a,ost=1;
 	...
 	obhod_v_shirinu(tree_a->root,tree_a->size);
        return 0;
}
